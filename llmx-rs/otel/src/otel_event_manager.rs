...
impl OtelEventManager {
    ...
    pub fn conversation_starts(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "llmx.conversation_starts",
    ...
    pub fn sandbox_assessment(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "llmx.sandbox_assessment",
            event.timestamp = %timestamp(),
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
            auth_mode = self.metadata.auth_mode,
    ...
    pub fn sandbox_assessment_latency(&self, call_id: &str, duration: Duration) {
        tracing::event!(
            tracing::Level::INFO,
            event.name = "llmx.sandbox_assessment_latency",
            event.timestamp = %timestamp(),
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
            auth_mode = self.metadata.auth_mode,
