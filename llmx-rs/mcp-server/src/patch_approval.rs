...
pub struct PatchApprovalElicitRequestParams {
    ...
    pub llmx_reason: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub llmx_grant_root: Option<PathBuf>,
    pub llmx_changes: HashMap<PathBuf, FileChange>,
}
...
pub(crate) async fn handle_patch_approval_request(
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_patch_approval_response(event_id, on_response, llmx).await;
        });
    }
}
