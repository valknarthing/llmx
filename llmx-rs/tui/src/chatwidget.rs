...
const NUDGE_MODEL_SLUG: &str = "gpt-5-llmx-mini";
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: llmx_core::protocol::SessionConfiguredEvent) {
        ...
        // Ask llmx-core to enumerate custom prompts for this session.
    ...
    pub(crate) fn new(
        ...
        let llmx_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), conversation_manager);
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<llmx_core::LlmxConversation>,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::LlmxOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::Undo => {
                self.app_event_tx.send(AppEvent::LlmxOp(Op::Undo));
            }
            SlashCommand::Diff => {
            ...
            SlashCommand::TestApproval => {
                ...
                self.app_event_tx.send(AppEvent::LlmxEvent(Event {
                    id: "1".to_string(),
    ...
    pub(crate) fn handle_llmx_event(&mut self, event: Event) {
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        match msg {
            ...
            _ => {
                tracing::trace!("handle_llmx_event: {:?}", msg);
            }
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::LlmxOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        for choice in choices.iter() {
            ...
            let show_warning =
                preset.model.starts_with("gpt-5-llmx") && effort == ReasoningEffortConfig::High;
    ...
    fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
        self.app_event_tx
            .send(AppEvent::LlmxOp(Op::OverrideTurnContext {
                cwd: None,
                approval_policy: None,
                sandbox_policy: None,
                        ...
                        && self.windows_world_writable_flagged()
                    {
                        ...
                        let (sample_paths, extra_count, failed_scan) =
                            match llmx_windows_sandbox::preflight_audit_everyone_writable(
                                &self.config.cwd,
                                &env_map,
                                Some(self.config.llmx_home.as_path()),
                            ) {
    ...
    /// Forward an `Op` directly to llmx.
    pub(crate) fn submit_op(&self, op: Op) {
        // Record outbound operation for session replay fidelity.
        crate::session_log::log_outbound_op(&op);
        if let Err(e) = self.llmx_op_tx.send(op) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(
                move |tx: &AppEventSender| {
                    tx.send(AppEvent::LlmxOp(Op::Review {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::LlmxOp(Op::Review {
                        review_request: ReviewRequest {
                            prompt: format!(
