...
pub(crate) fn maybe_init(config: &Config) {
    let enabled = std::env::var("LLMX_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("LLMX_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
...
pub(crate) fn log_inbound_app_event(event: &AppEvent) {
    ...
    match event {
        AppEvent::LlmxEvent(ev) => {
            write_record("to_tui", "llmx_event", ev);
        }
        AppEvent::NewSession => {
            let value = json!({
                "ts": now_ts(),
