...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let cfg = test_config();
    let conversation_manager = Arc::new(ConversationManager::with_auth(LlmxAuth::from_api_key(
        "test",
    )));
    let auth_manager = AuthManager::from_auth_for_testing(LlmxAuth::from_api_key("test"));
...
fn rate_limit_switch_prompt_skips_when_on_lower_cost_model() {
    ...
    chat.auth_manager =
        AuthManager::from_auth_for_testing(LlmxAuth::create_dummy_chatgpt_auth_for_testing());
...
fn rate_limit_switch_prompt_shows_once_per_session() {
    let auth = LlmxAuth::create_dummy_chatgpt_auth_for_testing();
    let (mut chat, _, _) = make_chatwidget_manual();
...
fn slash_undo_sends_op() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::LlmxOp(Op::Undo)) => {}
        other => panic!("expected AppEvent::LlmxOp(Op::Undo), got {other:?}"),
    }
}
...
fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual();
    let rollout_path = PathBuf::from("/tmp/llmx-test-rollout.jsonl");
...
fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::LlmxOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::LlmxOp(Op::Review { review_request }) => {
            assert_eq!(
...
fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::LlmxOp should be sent
...
fn model_selection_popup_snapshot() {
    ...
    chat.config.model = "gpt-5-llmx".to_string();
...
fn model_reasoning_selection_popup_snapshot() {
    ...
    let preset = builtin_model_presets(None)
        .into_iter()
        .find(|preset| preset.model == "gpt-5-llmx")
        .expect("gpt-5-llmx preset");
...
async fn binary_size_transcript_snapshot() {
    // the snapshot in this test depends on gpt-5-llmx. Skip for now. We will consider
    // creating snapshots for other models in the future.
    if OPENAI_DEFAULT_MODEL != "gpt-5-llmx" {
        return;
    }
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual();
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a LlmxOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::LlmxOp(Op::PatchApproval { id, decision }) = app_ev {
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a LlmxOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::LlmxOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected LlmxOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto llmx_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to llmx channel");
    match forwarded {
