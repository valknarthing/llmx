...
pub(crate) fn spawn_agent(
    ...
    let (llmx_op_tx, mut llmx_op_rx) = unbounded_channel::<Op>();
    ...
    tokio::spawn(async move {
        let NewConversation {
            ...
            Err(e) => {
                ...
                tracing::error!("failed to initialize llmx: {e}");
                return;
            }
        };
        ...
        app_event_tx_clone.send(AppEvent::LlmxEvent(ev));

        let conversation_clone = conversation.clone();
        tokio::spawn(async move {
            while let Some(op) = llmx_op_rx.recv().await {
        ...
        while let Ok(event) = conversation.next_event().await {
            app_event_tx_clone.send(AppEvent::LlmxEvent(event));
        }
    });
...
pub(crate) fn spawn_agent_from_existing(
    conversation: std::sync::Arc<LlmxConversation>,
