...
// The standalone `llmx-tui` binary prints a short help message before the
...
pub async fn run_main(
    ...
    let log_file = log_file_opts.open(log_dir.join("llmx-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for llmx crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("llmx_core=info,llmx_tui=info,llmx_rmcp_client=info")
        })
    };
    ...
    let feedback = llmx_feedback::LlmxFeedback::new();
...
async fn run_ratatui_app(
    ...
    let auth_manager = AuthManager::shared(
        initial_config.llmx_home.clone(),
        false,
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        match find_conversation_path_by_id_str(&config.llmx_home, id_str).await? {
            ...
            None => {
                ...
                if let Err(err) = writeln!(
                    std::io::stdout(),
                    "No saved session found with ID {id_str}. Run `llmx resume` without an ID to choose from existing sessions."
                ) {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let llmx_home = config.llmx_home.clone();
        match LlmxAuth::from_auth_storage(&llmx_home, config.cli_auth_credentials_store_mode) {
