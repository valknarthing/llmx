...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.llmx, |ev| {
    ...
    let end_event = wait_for_event(&fixture.llmx, |ev| {
...
/// This test writes to a fallback credentials file in LLMX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire LLMX_HOME through the code.
#[serial(llmx_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("LLMX_HOME", temp_home.path().as_os_str());
