...
/// Writes an `auth.json` into the provided `llmx_home` with the specified parameters.
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "llmx_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/api/llmx", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    ...
    let mut config = load_default_config_for_test(&llmx_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_llmx_auth());
    let NewConversation {
        conversation: llmx,
        conversation_id,
        session_configured: _,
    ...
    // Note: The "include" field is only present when reasoning is configured.
    // This test focuses on ChatGPT auth headers, not reasoning features.
}
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match LlmxAuth::from_auth_storage(llmx_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => llmx_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No LlmxAuth found in llmx_home"),
            Err(e) => panic!("Failed to load LlmxAuth: {e}"),
        };
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-llmx-primary-used-percent", "12.5")
        .insert_header("x-llmx-secondary-used-percent", "40.0")
        .insert_header("x-llmx-primary-window-minutes", "10")
        .insert_header("x-llmx-secondary-window-minutes", "60")
        .insert_header("x-llmx-primary-reset-at", "1704069000")
        .insert_header("x-llmx-secondary-reset-at", "1704074400")
        .set_body_raw(sse_body, "text/event-stream");
    ...
    let conversation_manager = ConversationManager::with_auth(LlmxAuth::from_api_key("test"));
    let llmx = conversation_manager
        .new_conversation(config)
        .await
    ...
    let first_token_event =
        wait_for_event(&llmx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5-llmx in tests → 95% usable context window
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-llmx-primary-used-percent", "100.0")
        .insert_header("x-llmx-secondary-used-percent", "87.5")
        .insert_header("x-llmx-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-llmx-primary-window-minutes", "15")
        .insert_header("x-llmx-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let llmx_fixture = builder.build(&server).await?;
    let llmx = llmx_fixture.llmx.clone();
    ...
    let token_event = wait_for_event(&llmx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&llmx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&llmx, |event| {
        matches!(
            event,
    ...
    let expected_context_window_message = LlmxErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_llmx_auth() -> LlmxAuth {
    LlmxAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    ...
    // Init session with isolated llmx home.
    let llmx_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&llmx_home);
    config.model_provider = model_provider;
