...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestLlmx) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestLlmx, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.llmx, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestLlmx) {
    let event = wait_for_event(&test.llmx, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TaskComplete(_)
...
async fn wait_for_completion(test: &TestLlmx) {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            with_escalated_permissions: false,
            features: vec![],
            model_override: Some("gpt-5-llmx"),
            outcome: Outcome::Auto,
            expectation: Expectation::PatchApplied {
                target: TargetPath::Workspace("apply_patch_function.txt"),
                content: "function-apply-patch",
