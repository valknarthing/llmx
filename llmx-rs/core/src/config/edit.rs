...
pub fn apply_blocking(
    ...
    let config_path = llmx_home.join(CONFIG_TOML_FILE);
    ...
    std::fs::create_dir_all(llmx_home).with_context(|| {
        format!(
    ...
    let tmp = NamedTempFile::new_in(llmx_home)?;
...
pub async fn apply(
    ...
    let llmx_home = llmx_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&llmx_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(llmx_home: &Path) -> Self {
        Self {
            llmx_home: llmx_home.to_path_buf(),
            profile: None,
            edits: Vec::new(),
        }
    }
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.llmx_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let contents =
            std::fs::read_to_string(llmx_home.join(CONFIG_TOML_FILE)).expect("read config");
        let expected = r#"model = "gpt-5-llmx"
...
"#;
    ...
    fn blocking_set_model_preserves_inline_table_contents() {
        ...
        std::fs::write(
            llmx_home.join(CONFIG_TOML_FILE),
            r#"profile = "fast"
...
"#,
        ...
        let raw = std::fs::read_to_string(llmx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
";
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !llmx_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
        ...
        let updated_expected = r#"model = "gpt-5-llmx"
model_reasoning_effort = "high"
"#;
        ConfigEditsBuilder::new(llmx_home)
            .set_model(Some("gpt-5-llmx"), Some(ReasoningEffort::High))
        ...
        contents = std::fs::read_to_string(llmx_home.join(CONFIG_TOML_FILE)).expect("read config");
