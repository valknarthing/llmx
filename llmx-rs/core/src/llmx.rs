...
/// The high-level interface to the Llmx system.
...
/// Wrapper returned by [`Llmx::spawn`] containing the spawned [`Llmx`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct LlmxSpawnOk {
    pub llmx: Llmx,
...
impl Llmx {
    /// Spawn a new [`Llmx`] and initialize the session.
    pub async fn spawn(
        config: Config,
        auth_manager: Arc<AuthManager>,
        conversation_history: InitialHistory,
        session_source: SessionSource,
    ) -> LlmxResult<LlmxSpawnOk> {
...
pub(crate) struct TurnContext {
    ...
    pub(crate) llmx_linux_sandbox_exe: Option<PathBuf>,
...
impl Session {
    fn make_turn_context(
        ...
        TurnContext {
            ...
            llmx_linux_sandbox_exe: config.llmx_linux_sandbox_exe.clone(),
...
mod handlers {
    ...
    pub async fn list_mcp_tools(sess: &Session, config: &Arc<Config>, sub_id: String) {
        ...
        let templates_fut = sess
            .services
            .mcp_connection_manager
            .list_all_resource_templates();
        let (auth_status_entries, resources, resource_templates) =
            tokio::join!(auth_statuses_fut, resources_fut, templates_fut);
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        llmx_linux_sandbox_exe: parent_turn_context.llmx_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
    };
...
pub(crate) async fn run_task(
    ...
    // Although from the perspective of llmx.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        .await
        {
            ...
            Err(LlmxErr::TurnAborted {
                ...
                return Err(LlmxErr::TurnAborted {
                    dangling_artifacts: processed_items,
                });
            }
            Err(LlmxErr::Interrupted) => return Err(LlmxErr::Interrupted),
            Err(LlmxErr::EnvVar(var)) => return Err(LlmxErr::EnvVar(var)),
            Err(e @ LlmxErr::Fatal(_)) => return Err(e),
            Err(e @ LlmxErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(e);
            }
            Err(LlmxErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, rate_limits).await;
                }
                return Err(LlmxErr::UsageLimitReached(e));
            }
            Err(LlmxErr::UsageNotIncluded) => return Err(LlmxErr::UsageNotIncluded),
            Err(e @ LlmxErr::QuotaExceeded) => return Err(e),
            Err(e @ LlmxErr::RefreshTokenFailed(_)) => return Err(e),
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = match e {
                        LlmxErr::Stream(_, Some(delay)) => delay,
                        _ => backoff(retries),
                    };
                    warn!(
...
async fn try_run_turn(
    ...
    let mut output: FuturesOrdered<BoxFuture<LlmxResult<ProcessedResponseItem>>> =
        FuturesOrdered::new();
            ...
            ResponseEvent::OutputItemDone(item) => {
                ...
                match ToolRouter::build_tool_call(sess.as_ref(), item.clone()) {
                    ...
                    Err(FunctionCallError::Fatal(message)) => {
                        return Err(LlmxErr::Fatal(message));
                    }
                }
            }
...
fn mcp_init_error_display(
        ...
        && http_headers.as_ref().map(HashMap::is_empty).unwrap_or(true)
    {
        ...
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = LLMX_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `llmx mcp login {server_name}`."
        )
...
mod tests {
    ...
    pub(crate) fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let llmx_home = tempfile::tempdir().expect("create temp dir");
