...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = llmx_home.clone();

    let opts = ServerOptions {
        llmx_home: server_home,
    ...
    // Validate auth.json
    let auth_path = llmx_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_llmx_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let llmx_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_llmx_home = first_tmp.path().to_path_buf();
    ...
    let second_llmx_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        llmx_home: second_llmx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: llmx_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
