//! Defines the protocol for an LLMX session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for LLMX:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::llmx_conversation::LlmxConversation`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Request Llmx to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request a code review from the agent.
    Review { review_request: ReviewRequest },

    /// Request to shut down llmx instance.
    Shutdown,
...
/// running the command proposed by Llmx.
#[derive(
    Debug,
    Clone,
    Copy,
    ...
    fn serialize_event() -> Result<()> {
        ...
        let event = Event {
            ...
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                ...
                model: "llmx-mini-latest".to_string(),
