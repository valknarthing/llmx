...
//!
//! If the keyring is not available or fails, we fall back to LLMX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
/// Determine where Llmx should store and read MCP credentials.
...
mod tests {
    ...
    impl TempLlmxHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create LLMX_HOME temp dir");
            unsafe {
                std::env::set_var("LLMX_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempLlmxHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("LLMX_HOME");
            }
        }
    }
